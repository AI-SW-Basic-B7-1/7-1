...
- **문제 정의**: 로그인한 사용자가 웹에서 AI와 대화하고, 이전 질문과 답변을 다시 찾을 수 있는 서비스를 제공합니다.
- **대상 사용자**: 브라우저에서 개인 계정으로 AI 질문·답변과 대화 이력을 관리하려는 사용자입니다.
- **핵심 시나리오**: 회원가입·로그인 후 질문을 보내면 서버가 Gemini에 전달합니다. 최근 대화 문맥으로 후속 질문에 답하고, 질문과 응답을 사용자 대화방에 저장해 다시 조회합니다.
- **GitHub Repository**: [AI-SW-Basic-B7-1/7-1](https://github.com/AI-SW-Basic-B7-1/7-1)
...
[ AWS EC2 t2.micro (Ubuntu 22.04 LTS) ]
 ├── [ Nginx 리버스 프록시 ] (Port 80 -> Port 8000 요청 전달, /static 경로 포함)
