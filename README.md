...
*   **Day 1 (독립 모듈 세팅 & AI PoC)**:
    ...
    *   [차종민] 코디세이 AI API 단독 호출 PoC 스크립트 작성 및 8초 타임아웃 사전 검증
    *   [고준석] 비밀번호 `bcrypt` 해싱 및 JWT 토큰 생성 유틸리티 함수 작성
    *   [박범규] FastAPI 기본 프로젝트 골격 생성 및 SQLite 스키마(`users`, `chat_logs`) 세팅
    *   [이준혁] 반응형 채팅 인터페이스 HTML/CSS 와이어프레임 작성
*   **Day 2 (각자 담당 코어 API 및 로직 완성)**:
    *   [고준석] 회원가입/로그인 엔드포인트 및 `get_current_user` 인증 의존성 완성
    *   [박범규] 대화 로그 DB 저장 함수, `GET /api/me/chats` 구현, 표준 4대 로깅 포맷터 적용
    *   [이준혁] 로그인/회원가입 모달 UI 완성, 토큰 저장 및 백엔드 비동기 통신 연동
    *   [차종민] 슬라이딩 윈도우 문맥 조립 함수, 8초 타임아웃 예외처리 및 Mock AI 엔진 구현
*   **Day 3 (전체 결합 - Alpha Release)**:
    *   인증 의존성 + 채팅 엔드포인트(`POST /api/chat`) 라우터 결합
    *   UI에서 질문 입력 시 `토큰 검증 -> 백엔드 수신 -> AI 호출 -> DB 저장 -> 화면 렌더링` 전 과정 E2E 통합
    *   통합 PR 생성, 코드 리뷰 후 `develop` 브랜치 머지
*   **Day 4 (안정성 강화, 입력 검증 & 프로토타입 점검)**:
    *   공백 입력 및 500자 초과 비정상 입력 차단 (400/422 검증)
    *   타임아웃/API 에러 시 504 에러 및 친절한 안내 메시지 UI 연동
    *   `scripts/check_logs.sql`로 대화 로그 저장 상태 검증
    *   AWS EC2 인프라 세팅(Nginx, 2GB Swap, Systemd) 및 외부 접속 시연 점검
