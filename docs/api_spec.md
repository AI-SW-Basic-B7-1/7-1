...
앱 오류의 공통 형식은 `{"detail":"한국어 문자열"}`입니다. 422의 실제 메시지는 항상 `요청 데이터 형식이 올바르지 않습니다.`이며 Pydantic 기본 오류 배열이나 필드별 메시지를 반환하지 않습니다. 프록시의 HTML 오류는 앱 계약 밖이므로 클라이언트가 안전한 일반 문구로 처리합니다.
...
| 422 | 원문 500자 초과, 필수 필드 누락, 잘못된 형식/ID | `요청 데이터 형식이 올바르지 않습니다.` |
