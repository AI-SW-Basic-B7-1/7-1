...
현행 구현은 2026-09-23 `develop`의 `755e7e2` 기준입니다. 반려동물 국내여행은 서비스 기획이며 현재 화면은 범용 AI Assistant입니다. 관광 데이터·장소 카드·수요 지표·지도는 아직 구현되지 않았습니다.
...
프론트 단위·계약 테스트는 백엔드 설치와 무관하게 실행할 수 있습니다. API·정렬·그룹화·상태 격리·정적 접근성 계약을 함께 검증합니다.

```powershell
node --test tests/frontend/*.test.mjs
```

현재 팀 백엔드는 2절과 3절의 계약을 구현하고 있습니다. 의존성·환경 변수 설정 후에는 다음 명령으로 전체 서비스를 실행하고 `http://127.0.0.1:8000`에서 검증합니다.
...
`conversation_id` 대화방 UI의 [#46](https://github.com/AI-SW-Basic-B7-1/7-1/pull/46)과 DB 계약의 [#37](https://github.com/AI-SW-Basic-B7-1/7-1/pull/37)은 이미 develop에 병합되었습니다. 위 2026-09-16 브라우저 대역 결과와 현재 자동 테스트 결과는 서로 구분합니다. 최신 검증은 [평가 가이드](evaluation_guide.md)를 보며, 실제 두 계정 브라우저·실 AI·재접속 이력 시연은 별도 확인 대상입니다.
...
## 8. 여행·지도 확장의 접합 기준 (미구현)

후속 설계는 [반려동물 여행 명세](pet_travel_spec.md)를 따릅니다. 지역 수요는 비교 보조 지표이고, 장소별 동반 가능 여부나 실시간 혼잡도를 뜻하지 않습니다.

- 현재 응답에는 장소·좌표 배열이 없으므로 서버 구조화 계약과 저장/재로그인 복원 방식을 먼저 합의합니다. `answer` 문장에서 좌표를 뽑거나 AI 좌표를 사용하지 않습니다.
- 확인한 좌표만 지도/마커로 표시하며 누락 시 텍스트 주소·조건·출처를 제공합니다. 지도 장애가 채팅·이력을 막지 않아야 합니다.
- 브라우저 지도 키는 보일 수 있습니다. Git 밖에서 주입하고 웹사이트·API 제한을 적용하며 서버 키와 분리합니다.
- Maps JavaScript API 등 외부 SDK 채택 시 현재 동일 출처 정적 자원 테스트를 필요한 공식 호스트 범위에서 갱신합니다. 기존 API·인증·textContent 검증은 유지합니다.
- 계정/방 전환과 늦은 응답 무효화에 장소·수요·지도 상태도 포함합니다. 키보드 접근·작은 화면·지도 없는 읽기 경로를 검증합니다.
- 외부 지도 링크만 제공한 결과는 Google Maps API 구현 완료로 표시하지 않습니다.
