# AI Assistant — 반려동물 국내여행 챗봇 중기 프로젝트 계획

> AI/SW 기초 · Term Project · 필수 · 학습시간 **120시간**
>
> 문서 성격: 초기 4일 MVP 이후의 현재 상태와 후속 개발 방향을 정리한 중기 계획입니다. 프로젝트 시작 당시의 일정·가정·역할 원문은 [4일 프로토타입 초기 프로젝트 계획](project_plan.md)에 보존합니다.
>
> 현행 구현 기준: `develop`의 `755e7e2` (2026-09-23). 120시간은 과제 학습시간이며 초기 4일 내부 계획과 별개입니다. 실제 투입시간이나 전체 기능 완료를 뜻하지 않습니다.
...
[초기 프로젝트 계획](project_plan.md)은 당시 합의한 4일 일정, 코디세이/OpenAI 호환 API, 내장 Mock과 초기 스키마 가정을 수정하지 않고 기록으로 유지합니다. 현재 구현 여부는 이 문서 2절과 평가 가이드의 증빙으로 판단합니다. 초기 계획의 체크 항목이나 목표 문구를 120시간 과제 또는 현재 기능의 완료 증빙으로 사용하지 않습니다.

현행 런타임은 [#29](https://github.com/AI-SW-Basic-B7-1/7-1/pull/29) 이후 Gemini를 사용하며, DB는 `users → conversations → chat_logs` 구조입니다. 초기 계획과 현재 구현이 다를 때는 README·현행 API 계약·코드·검증 결과를 우선합니다.
